...
class BallzGen {
    ...
    class Particle {
        ...
        void draw(int mouseX, int mouseY) {
            ...
            Ballz.drawBorderedCircle(this.x + xx, this.y + yy, this.size, 0, 553648127);


            float distance = (float) Ballz.distance(this.x + xx, this.y + yy, mouseX, mouseY);

            if (distance < 70) {
                float alpha1 = Math.min(1.0f, Math.min(1.0f, 1.0f - distance / 70));
        ...
        public
        void resetPosSize() {
            this.x = this.random.nextInt(BallzGen.this.width);
            this.y = this.random.nextInt(BallzGen.this.height);
            this.size = genRandom(1.0F, 4.0F);
        }
